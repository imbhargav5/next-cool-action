...
This is how a cool action is created. Providing a validation input schema to the function via [`inputSchema()`](/docs/define-actions/instance-methods#inputschema), we're sure that data that comes in is type safe and validated.
...
export default function Login() {
  return (
    <button
      onClick={async () => {
        // Typecool action called from client.
