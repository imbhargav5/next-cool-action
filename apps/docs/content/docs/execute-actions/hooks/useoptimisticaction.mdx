...
- `coolActionFn`: the cool action that will be called via `execute()` or `executeAsync()` functions.
...
- `execute()`: an action caller with no return. Input is the same as the cool action you passed to the hook.
- `executeAsync()`: an action caller that returns a promise with the return value of the cool action. Input is the same as the cool action you passed to the hook.
